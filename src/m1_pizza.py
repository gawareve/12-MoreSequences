...
def run_test_draw_points_on_circle():
    ...
    window = rg.RoseWindow(400, 400, title)
    circle = rg.Circle(rg.Point(150, 300), 15)
    draw_points_on_circle(window, circle, 20, 'teal')
    window.close_on_mouse_click()
...
def run_test_fancy_polygon():
    ...
    fancy_polygon(window, circle, 25, 7, 'light blue', 5)
    window.close_on_mouse_click()
    # -------------------------------------------------------------------------
    # Test 4:  (YOU write THIS test).
...
def fancy_polygon(window, circle, number_of_lines, hops_to_next_point, color,
    ...
    for k in range(len(points)):
            ...
            end= k +hops_to_next_point
            if end>= len(points):
                end=end-number_of_lines
            line=rg.Line(points[k],points[end])
            line.arrow='last'
            line.color=color
            line.thickness=thickness
            line.attach_to(window)
